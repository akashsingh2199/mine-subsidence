...
A synthetic 16-node mine sensor mesh was defined as a CSV dataset.
CSV rows were grouped into complete chronological timestamp frames.
A Node.js backend was created to load, normalize, replay, pause, reset, seek, and speed-control those frames.
Socket.IO was added to stream status and frames to browser clients.
An Axios adapter was added for the separate ML service.
A FastAPI service was added to load serialized scikit-learn models and expose health and prediction endpoints.
The React dashboard was connected to live backend frames instead of depending only on static demo values.
Ground-truth labels were kept separate from model predictions.
Replay controls, a 4x4 sensor mesh, selected-node metrics, alerts, a local-coordinate deformation map, and live charts were added.
Offline ML fallback was added so CSV readings remain visible when the model service is unavailable.
Seek behavior was made responsive with optimistic state, a 60 ms debounce, and a 1.5 second confirmation timeout.
...
Isolation Forest anomaly detection, where -1 means anomaly.
Binary risk probability.
Final status rules, in order:

active_subsidence becomes ACTIVE SUBSIDENCE.
Anomaly plus risk probability at or above the threshold becomes HIGH RISK / INVESTIGATION REQUIRED.
early_subsidence becomes EARLY SUBSIDENCE.
All other results become NORMAL.
...
End-to-End Flow
The ML service loads the joblib model bundle.
The backend reads and validates the CSV into chronological complete frames.
Vite serves the React application.
The browser connects to Socket.IO.
The backend sends status and the current frame.
The backend enriches each node with ML output when possible.
The hook adapts the frame and derives display status from ground truth.
The dashboard updates counts, mesh, selected-node values, map, alerts, and charts.
Start, pause, reset, seek, day jumps, and speed changes send commands back to the simulator.
...
Add automated tests for CSV grouping, frame completeness, seek bounds, speed validation, offline ML, and socket adaptation.
Add an example environment file documenting the port contract.
Document or reproduce how the joblib model bundle was trained.
Remove or label unused mock fixtures.
Standardize packet-loss units and formatting.
Add persistence and a real ingestion path for production use.
Add routing if sidebar navigation is intended to be functional.
Add client reconnection/loading tests.
Split the frontend bundle if the build-size warning affects deployment.
